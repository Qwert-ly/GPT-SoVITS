...
def predict(session, onnx_input: Dict[str, Any],
    ...
    # 2. 直接获取概率矩阵
    probs = session.run([], onnx_inputs)[0]

    # 3. 使用numpy向量化操作，避免Python循环
    preds = np.argmax(probs, axis=1)

    # 4. 使用numpy的高效索引操作获取最大概率值
    max_probs = np.take_along_axis(probs, preds[:, np.newaxis], axis=1).flatten()

    return [labels[pred] for pred in preds], max_probs.tolist()
