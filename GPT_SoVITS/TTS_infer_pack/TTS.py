...
import math, gc, random, ffmpeg, torchaudio
...
class TTS_Config:
    default_configs = {
        "v1": {
            ...
            "is_half": False,
            "version": "v1",
            "t2s_weights_path": ROOT + "s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
            "vits_weights_path": ROOT + "s2G488k.pth",
            "cnhuhbert_base_path": ROOT + "chinese-hubert-base",
            "bert_base_path": ROOT + "chinese-roberta-wwm-ext-large",
        },
        "v2": {
            "device": "cpu",
            "is_half": False,
            "version": "v2",
            "t2s_weights_path": ROOT + "gsv-v2final-pretrained/s1bert25hz-5kh-longer-epoch=12-step=369668.ckpt",
            "vits_weights_path": ROOT + "gsv-v2final-pretrained/s2G2333k.pth",
            "cnhuhbert_base_path": ROOT + "chinese-hubert-base",
            "bert_base_path": ROOT + "chinese-roberta-wwm-ext-large",
        },
        "v3": {
            ...
            "is_half": False,
            "version": "v3",
            "t2s_weights_path": ROOT + "s1v3.ckpt",
            "vits_weights_path": ROOT + "s2Gv3.pth",
            "cnhuhbert_base_path": ROOT + "chinese-hubert-base",
            "bert_base_path": ROOT + "chinese-roberta-wwm-ext-large",
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        if "cuda" in str(self.device) and not torch.cuda.is_available():
            print(f"Warning: CUDA不适用，切换至CPU.")
...
def sola_algorithm(audio_fragments: List[torch.Tensor],
                   overlap_len: int,
                   ):
...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config]):
        ...
        self.text_preprocessor: TextPreprocessor = TextPreprocessor(self.bert_model,
                                                                    self.bert_tokenizer,
                                                                    self.configs.device)
    ...
    def init_cnhuhbert_weights(self, base_path: str):
        print(f"从{base_path}加载CNHuBERT权重")
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if not if_lora_v3:
            print(f"从{weights_path}加载VITS权重：{vits_model.load_state_dict(dict_s2['weight'], strict=False)}")

        else:
            print(
                f"从{weights_path}加载VITS预训练权重：{vits_model.load_state_dict(load_sovits_new(path_sovits_v3)['weight'], strict=False)}")
            ...
            print(f"从{weights_path}加载LORA权重：{vits_model.load_state_dict(dict_s2['weight'], strict=False)}")
    ...
    def init_t2s_weights(self, weights_path: str):
        print(f"从{weights_path}加载Text2Semantic权重")
    ...
    def init_bigvgan(self):
        ...
        self.bigvgan_model = BigVGAN.from_pretrained(f"{now_dir}/{ROOT}models--nvidia--bigvgan_v2_24khz_100band_256x",
    ...
    def _get_ref_spec(self, ref_audio_path):
        ...
        audio, sampling_rate = librosa.load(ref_audio_path, sr=int(self.configs.sampling_rate))
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        zero_wav = np.zeros(int(self.configs.sampling_rate * 0.3),
        ...
        with torch.no_grad():
            ...
            hubert_feature = self.cnhuhbert_model.model(wav16k.unsqueeze(0))["last_hidden_state"].transpose(1,
                                                                                                            2)  # .float()
    ...
    def run(self, inputs: dict):
        ...
        if not return_fragment:
            ...
            data, batch_index_list = to_batch(data,
            ...
            def make_batch(batch_texts):
                ...
                for text in tqdm(batch_texts):
                    phones, bert_features, norm_text = \
                        self.text_preprocessor.segment_and_extract_feature_for_text(text, text_lang,
                                                                                    self.configs.version)
                    if phones is None:
                        continue
                    batch_data.append({
            ...
            for item in data:
                ...
                if not self.configs.is_v3_synthesizer:
                    if speed_factor == 1.0:
                        ...
                        all_pred_semantic = torch.cat(pred_semantic_list).unsqueeze(0).unsqueeze(0).to(
                            self.configs.device)
                    ...
                    if parallel_infer:
                        ...
                        for i, idx in enumerate(tqdm(idx_list)):
                            ...
                            _pred_semantic = (
                                pred_semantic_list[i][-idx:].unsqueeze(0).unsqueeze(0))  # .unsqueeze(0)#mq要多unsqueeze一次
                            audio_fragment = self.v3_synthesis(_pred_semantic, phones, speed=speed_factor,
                                                               sample_steps=sample_steps)
    ...
    def v3_synthesis(self,
        ...
        while 1:
            ...
            cfm_res = self.vits_model.cfm.inference(fea, torch.LongTensor([fea.size(1)]).to(fea.device), mel2,
    ...
    def v3_synthesis_batched_infer(self,
        ...
        pred_spec = self.vits_model.cfm.inference(fea, torch.LongTensor([fea.size(1)]).to(fea.device), mel2,
                                                  sample_steps, inference_cfg_rate=0)
        ...
        audio = sola_algorithm(audio_fragments, overlapped_len * upsample_rate)
