...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    if prompt_language is not None and text_language is not None:
        if prompt_language in list(dict_language.keys()):
            prompt_text_update, prompt_language_update = {'__type__': 'update'}, sync(prompt_language)
        else:
            prompt_text_update = sync('')
            prompt_language_update = sync(i18n("中文"))
        if text_language in list(dict_language.keys()):
            text_update, text_language_update = {'__type__': 'update'}, sync(prompt_language)
        else:
            text_update = sync('')
            text_language_update = sync(i18n("中文"))
        visible_sample_steps = model_version == "v3"
        visible_inp_refs = model_version != "v3"

        # prompt_language,text_language,prompt_text,prompt_language,text,text_language,inp_refs,ref_text_free,

        yield {'__type__': 'update', 'choices': list(dict_language.keys())}, {'__type__': 'update', 'choices': list(
            dict_language.keys())}, prompt_text_update, prompt_language_update, text_update, text_language_update,\
            {"__type__": "update", "interactive": visible_sample_steps, "value": 32}, {"__type__": "update", "visible": visible_inp_refs},\
            {"__type__": "update", "interactive": model_version != "v3"}, {"__type__": "update", "value": i18n("模型加载中，请等待"), "interactive": False}
        ...
        yield {'__type__': 'update', 'choices': list(dict_language.keys())},\
            ...
            {"__type__": "update", "interactive": model_version != "v3"},\
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    gr.Markdown(
        value=i18n(
            "本软件以MIT协议开源, 作者不对软件具备任何控制力, 使用软件者、传播软件导出的声音者自负全责.") + "<br>" + i18n(
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Row():
                ...
                inp_refs = gr.File(label=i18n("辅参考音频(可选多个，或不选)"), file_count="multiple", visible=model_version != "v3")
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ref_text_free = gr.Checkbox(label=i18n("开启无参考文本模式。不填参考文本亦相当于开启。"), value=False,
                                                interactive=model_version != "v3", show_label=True)
                    gr.Markdown(i18n("使用无参考文本模式时建议使用微调的GPT") + "<br>" + i18n(
    ...
    with gr.Group():
        ...
        with gr.Row():
            with gr.Column():
                ...
                sample_steps = gr.Radio(label=i18n("采样步数(仅对V3生效)"), value=32, choices=[4, 8, 16, 32])
            with gr.Row():
            ...
            with gr.Column():
                with gr.Row():
                    ...
                    super_sampling = gr.Checkbox(label=i18n("音频超采样(仅对V3生效))"), value=False, interactive=True,
                                                 show_label=True)

                with gr.Row():
                    parallel_infer = gr.Checkbox(label=i18n("并行推理"), value=True, interactive=True, show_label=True)
                    split_bucket = gr.Checkbox(label=i18n("数据分桶(并行推理时会降低一点计算量)"), value=True,
        ...
        SoVITS_dropdown.change(change_sovits_weights, [SoVITS_dropdown, prompt_language, text_language],
                               [prompt_language, text_language, prompt_text, prompt_language, text, text_language, sample_steps, inp_refs, ref_text_free, inference_button])
