...
from basic_util import perf_check
...
def run(rank, n_gpus, hps):
    ...
    net_d = MultiPeriodDiscriminator(hps.model.use_spectral_norm).cuda(
        rank) if torch.cuda.is_available() else MultiPeriodDiscriminator(hps.model.use_spectral_norm).to(device)
    ...
    try:  # 如果能加载自动resume
        ...
        epoch_str = 1
        global_step = 0
        if hps.train.pretrained_s2G != "" and hps.train.pretrained_s2G is not None and os.path.exists(
        ...
        if hps.train.pretrained_s2D != "" and hps.train.pretrained_s2D != None and os.path.exists(
