...
path_sovits_v3 = ROOT+"s2Gv3.pth"
is_exist_s2gv3 = os.path.exists(path_sovits_v3)
pretrained_sovits_name = [ROOT+"s2G488k.pth",
                          ROOT+"gsv-v2final-pretrained/s2G2333k.pth", path_sovits_v3]
pretrained_gpt_name = [ROOT+"s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
...
def get_weights_names(GPT_weight_root, SoVITS_weight_root):
    ...
    for path in SoVITS_weight_root:
        for name in os.listdir(path):
            if name.endswith(".pth"): SoVITS_names.append(f"{path}/{name}")
    GPT_names = [i for i in pretrained_gpt_name]
    for path in GPT_weight_root:
        for name in os.listdir(path):
            if name.endswith(".ckpt"): GPT_names.append(f"{path}/{name}")
...
cnhubert_base_path = os.environ.get("cnhubert_base_path", ROOT+"chinese-hubert-base")
bert_path = os.environ.get("bert_path", ROOT+"chinese-roberta-wwm-ext-large")
...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    if if_lora_v3 and not is_exist_s2gv3:
        info = ROOT + "s2Gv3.pth" + i18n("SoVITS V3 底模缺失，无法加载相应 LoRA 权重")
        gr.Warning(info)
    ...
    if prompt_language is not None and text_language is not None:
        if prompt_language in list(dict_language.keys()):
            ...
            prompt_text_update = sync('')
            prompt_language_update = {'__type__': 'update', 'value': i18n("中文")}
        if text_language in list(dict_language.keys()):
            text_update, text_language_update = {'__type__': 'update'}, {'__type__': 'update', 'value': text_language}
        else:
            text_update = sync('')
            text_language_update = sync(i18n("中文"))
        if model_version == "v3":
        ...
        yield {'__type__': 'update', 'choices': list(dict_language.keys())},\
            {'__type__': 'update', 'choices': list(dict_language.keys())}, prompt_text_update, prompt_language_update, text_update, text_language_update,\
            {"__type__": "update", "visible": visible_sample_steps, "value": 32},\
            {"__type__": "update", "visible": visible_inp_refs},\
            {"__type__": "update", "value": False, "interactive": model_version != "v3"},\
            {"__type__": "update", "visible": model_version == "v3"}, \
    ...
    if "pretrained" not in sovits_path:
        try:
            ...
            pass
    vq_model = vq_model.half().to(device) if is_half else vq_model = vq_model.to(device)
    vq_model.eval()
    if not if_lora_v3:
        print(f"加载sovits_{model_version}", vq_model.load_state_dict(dict_s2["weight"], strict=False))
    else:
        print("加载sovits_v3pretrained_G",
        ...
        print(f"加载sovits_v3_lora{lora_rank}")
...
def init_bigvgan():
    ...
    bigvgan_model = bigvgan.BigVGAN.from_pretrained(f"{now_dir}/{ROOT}models--nvidia--bigvgan_v2_24khz_100band_256x", use_cuda_kernel=False)  # if True, RuntimeError: Ninja is required to load C++ extensions
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut=i18n("不切"), top_k=20,
                top_p=0.6, temperature=0.6, ref_free=False, speed=1, if_freeze=False, inp_refs=None, sample_steps=8,
                if_sr=False, pause_second=0.3):
    global cache
    if not ref_wav_path:
    ...
    for i_text, text in enumerate(texts):
        ...
        if model_version != "v3":
            ...
            audio = \
                vq_model.decode(pred_semantic, torch.LongTensor(phones2).to(device).unsqueeze(0), refers, speed=speed)[0][0]  # .cpu().detach().numpy()
        else:
            ...
            while 1:
                ...
                # set_seed(123)
                cfm_res = vq_model.cfm.inference(fea, torch.LongTensor([fea.size(1)]).to(fea.device), mel2,
            ...
            if not bigvgan_model: init_bigvgan()
    ...
    with gr.Group():
        ...
        with gr.Row():
            GPT_dropdown = gr.Dropdown(label=i18n("GPT模型列表"), choices=sorted(GPT_names, key=custom_sort_key),
                                       value=gpt_path, interactive=True, scale=14)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=13):
                ref_text_free = gr.Checkbox(
                    ...
                    value=False, interactive=model_version != "v3", show_label=True, scale=1)
                gr.Markdown(html_left(i18n("使用无参考文本模式时建议使用微调的GPT") + "<br>" + i18n(
                    "听不清参考音频说的啥(不晓得写啥)可以开。开启后无视填写的参考文本。")))
                prompt_text = gr.Textbox(label=i18n("参考音频的文本"), value="", lines=5, max_lines=5, scale=1)
            with gr.Column(scale=14):
                prompt_language = gr.Dropdown(label=i18n("参考音频的语种"), choices=list(dict_language.keys()), value=i18n("中文"),)
                inp_refs = gr.File(label=i18n(
                    "可选项：通过拖拽多个文件上传多个参考音频（建议同性），平均融合他们的音色。如不填写此项，音色由左侧单个参考音频控制。如是微调模型，建议参考音频全部在微调训练集音色内，底模不用管。"),
                    file_count="multiple") if model_version != "v3" else gr.File(label=i18n(
                    "可选项：通过拖拽多个文件上传多个参考音频（建议同性），平均融合他们的音色。如不填写此项，音色由左侧单个参考音频控制。如是微调模型，建议参考音频全部在微调训练集音色内，底模不用管。"),
                    file_count="multiple",
                    visible=False)
                sample_steps = gr.Radio(label=i18n("采样步数,如果觉得电,提高试试,如果觉得慢,降低试试"), value=32,
                                        choices=[4, 8, 16, 32], visible=True) if model_version == "v3" else gr.Radio(
                    label=i18n("采样步数,如果觉得电,提高试试,如果觉得慢,降低试试"), choices=[4, 8, 16, 32],
                    visible=False, value=32)
                if_sr_Checkbox = gr.Checkbox(label=i18n("v3输出如果觉得闷可以试试开超分"), value=False,
        ...
        with gr.Row():
            ...
            with gr.Column(scale=7):
                text_language = gr.Dropdown(
                    ...
                    choices=list(dict_language.keys()), value=i18n("中文"), scale=1
                )
                ...
                with gr.Row():
                    speed = gr.Slider(minimum=0.6, maximum=1.65, step=0.05, label=i18n("语速"), value=1,
                                      interactive=True, scale=1)
                    pause_second_slider = gr.Slider(minimum=0.1, maximum=0.5, step=0.01, label=i18n("句间停顿秒数"),
                                                    value=0.3, interactive=True, scale=1)
        ...
        inference_button.click(
            ...
            [inp_ref, prompt_text, prompt_language, text, text_language, how_to_cut, top_k, top_p, temperature,
             ref_text_free, speed, if_freeze, inp_refs, sample_steps, if_sr_Checkbox, pause_second_slider],
            [output],
        )
        SoVITS_dropdown.change(change_sovits_weights, [SoVITS_dropdown, prompt_language, text_language],
                               [prompt_language, text_language, prompt_text, prompt_language, text, text_language,
                                sample_steps, inp_refs, ref_text_free, if_sr_Checkbox, inference_button])
